...
def identity_for(spec: RemoteSpec) -> str:
    ...
    [@<prefix-hash>]'. Pure function of `spec` (no I/O), so both
    PDFConfig.embedding_model and warm_cli's `--model` override (which
    swaps `spec.model` but keeps the configured base_url) can share it
    rather than reimplementing the same string.
    ...
    host = urlsplit(spec.base_url).netloc or spec.base_url
...
def encode(texts: list[str], spec: RemoteSpec, *, prefix: str = "") -> Any:
    ...
    (mirrors why encode() normalizes there rather than trusting fastembed:
    servers differ in whether they return unit vectors).
