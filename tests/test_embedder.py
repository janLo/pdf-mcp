...
class TestRemoteDispatch:
    ...
    def _spec(self, **overrides):
        ...
        defaults = dict(base_url="http://localhost:8000/v1", model="bge-small")
    ...
    def test_encode_applies_document_prefix(self):
        import pdf_mcp.embedder as emb

        emb.configure_remote(self._spec(document_prefix="passage: "))
        with patch("pdf_mcp.remote_embedder.encode") as mock_encode:
            mock_encode.return_value = np.array([[1.0]], dtype=np.float32)
            emb.encode(["hello"], "openai:localhost:8000/bge-small")
        assert mock_encode.call_args.kwargs["prefix"] == "passage: "

    def test_encode_query_applies_query_prefix(self):
        import pdf_mcp.embedder as emb

        emb.configure_remote(
            self._spec(document_prefix="passage: ", query_prefix="query: ")
        )
        with patch("pdf_mcp.remote_embedder.encode") as mock_encode:
            mock_encode.return_value = np.array([[1.0]], dtype=np.float32)
            emb.encode_query("hi", "openai:localhost:8000/bge-small")
...
class TestIsBgeSmallCompatible:
    """embedder.is_bge_small_compatible -- the heuristic
    PDFConfig.confidence_threshold uses to decide whether the bge-small-
    tuned 0.5 default applies (issue #46)."""

    def test_bare_fastembed_default_model(self):
        import pdf_mcp.embedder as emb

        assert emb.is_bge_small_compatible("BAAI/bge-small-en-v1.5") is True

    def test_bare_fastembed_other_model(self):
        import pdf_mcp.embedder as emb

        assert (
            emb.is_bge_small_compatible("snowflake/snowflake-arctic-embed-s") is False
        )

    def test_remote_identity_naming_bge_small(self):
        import pdf_mcp.embedder as emb

        assert (
            emb.is_bge_small_compatible("openai:localhost:8000/bge-small-en-v1.5")
            is True
        )

    def test_remote_identity_naming_a_different_model(self):
        import pdf_mcp.embedder as emb

        assert (
            emb.is_bge_small_compatible("openai:localhost:8000/nomic-embed-text")
            is False
        )
    ...
    def test_case_insensitive(self):
        ...
        assert emb.is_bge_small_compatible("BGE-SMALL-EN-V1.5-Q4_K_M") is True
