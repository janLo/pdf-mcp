...
- `features` (object):
  ...
  - `search.embedding_model` (string, conditional) — present **only** when semantic search is available; omitted otherwise. For the remote (`openai`) backend this is the namespaced cache identity (`openai:<host>[:<port>]/<model>[@<prefix-hash>]`), not the bare model name.
