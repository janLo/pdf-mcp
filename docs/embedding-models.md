...
- **`intfloat/multilingual-e5-large`** (2.2 GB, 1024-dim, 100+ languages) — **run against a German corpus (119 scenarios)**: MRR 0.632 vs bge-small's 0.383 on natural-language German queries (+0.249), the best of everything tested there. 490-page cold embed took ~13 min on CPU and warm query p50 landed at ~1.8x bge-small's, over this repo's own 1.5x latency gate. Run raw (no `query:`/`passage:` prefix — the production path applies none). See [`german_embedding_results.md`](../benchmark_data/german_embedding_results.md).
- **`jinaai/jina-embeddings-v2-base-de`** (320 MB, 768-dim, German) — **loads and runs with a one-line workaround**: fails under fastembed's hardcoded `onnxruntime.GraphOptimizationLevel.ORT_ENABLE_ALL` (a `SimplifiedLayerNormFusion` graph-optimizer pass chokes on this model's exported graph), but loads and embeds correctly one optimization level down (`ORT_ENABLE_EXTENDED`), verified standalone against the raw ONNX file. Run this way against the same German corpus: MRR 0.565 (+0.182 vs bge-small), 356s cold embed, p50 ~1.7x bge-small's — also over the latency gate, but a real, working candidate. See [`german_embedding_results.md`](../benchmark_data/german_embedding_results.md) for the mechanism.
- **`sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`** / **`-mpnet-base-v2`** — run against the same German corpus; both score modestly *below* bge-small (0.340 and 0.313 vs 0.383) despite being multilingual. MiniLM is markedly cheaper to embed (49s vs 140s cold) for a small quality cost. Bigger/multilingual doesn't automatically win here, the same conclusion the English large-model screen already reached.

If you need any of these (long contexts, multilingual, larger English models), pin via BYOM and validate the retrieval yourself before depending on it. For non-English corpora specifically, see [`german_embedding_results.md`](../benchmark_data/german_embedding_results.md) — no model tested there (local or remote) is a clean, latency-gate-passing drop-in for German yet, though `multilingual-e5-large` and `jina-embeddings-v2-base-de` both meaningfully close the gap and, surprisingly, beat the two remote models (`bge-m3`, `Qwen3-Embedding-0.6B`) also tested there.
